...
Функция ПолучитьСписокИменЛоггеров() Экспорт
	
	МассивИменЛоггеров = Новый Массив;
	
	Если мНастройкиЛогирования = Неопределено Тогда
		Возврат МассивИменЛоггеров;
	Иначе
		Для каждого ключЗначение из мНастройкиЛогирования Цикл
			МассивИменЛоггеров.Добавить(ключЗначение.Ключ);
		КонецЦикла;	
	КонецЕсли;	
...
Процедура ОбработатьСтрокуНастроек(Знач СтрокаНастроек)
	...
	Если Найти(СтрокаНастроек, "@tree") > 0 Тогда
		Логгер.МенеджерЛогов().ИспользоватьУровниВложенностиВызовов(Истина);//устанавливаем иерархическое логирование
		Возврат;
	КонецЕсли;	
	
	Если Найти(СтрокаНастроек,"@DetailCollection") > 0 Тогда
		Логгер.МенеджерЛогов().УстановитьДетализациюКоллекций(Истина);//устанавливаем вывод детализацию коллекций
		Возврат;
	КонецЕсли;	
...
Функция КлючНастройкиХранилища() Экспорт
	Возврат "НастройкиЛогированияЛог";
КонецФункции

Функция ИмяОбщегоВиртуальногоПользователя() Экспорт
	Возврат "НастройкиЛогированияЛог";
КонецФункции
